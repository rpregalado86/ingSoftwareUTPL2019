...
public class MetodoRutas {
    ...
    String resultadoRutas [] = new String [7];
    String [][]datos;
    
    public MetodoRutas () throws IOException{
        verificarArchivo(); 
    }
    ...
    private void leerArchivo() throws FileNotFoundException, IOException{
        ...
        if(numeroRegistros==0)
            System.out.println("ARCHIVO RUTAS.TXT VACIO");
        else{
            datos = new String[numeroRegistros][7];
            int posicion=0;
            String line = null;
    ...
    public String [] ObtenerRuta(String origen, String destino) throws IOException{
        leerArchivo();
        int encontrado=0;
        boolean enc=false;
        for (int i = 0; i < datos.length; i++) {
                if(datos[i][3].equalsIgnoreCase(origen)&&datos[i][4].equalsIgnoreCase(destino)){
                    encontrado=i;
                    enc=true;
                    break;
                }
        ...
        if(enc!=false){
            resultadoRutas [0] =datos[encontrado][0];
            resultadoRutas [1] =datos[encontrado][1];
            resultadoRutas [2] =datos[encontrado][2];
            resultadoRutas [3] =datos[encontrado][3];
            resultadoRutas [4] =datos[encontrado][4];
            resultadoRutas [5] =datos[encontrado][5];
            resultadoRutas [6] =datos[encontrado][6];
        }else{
            resultadoRutas [0] =null;
        }
